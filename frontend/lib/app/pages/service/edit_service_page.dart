...
class _EditServicePageState extends State<EditServicePage> {
  ...
  bool areTarifListsDifferent({
    required List<ServiceTarifModel> list1,
    required List<ServiceTarifModel?> list2,
  }) {
    // Vérifier si les tailles sont différentes
    if (list1.length != list2.length) return true;

    for (int i = 0; i < list1.length; i++) {
      if (list1[i].minQuantity != list2[i]!.minQuantity ||
          list1[i].maxQuantity != list2[i]!.maxQuantity ||
          list1[i].prix != list2[i]!.prix) {
        return true;
      }
    }
  ...
  Future<void> editService() async {
    ...
    List<ServiceTarifModel>? newTarif;
    PaysModel? newCountry;
    ServiceType? newType;
    NatureService? newNature;
    double? newPrix;
    if (libelle != initialService.libelle) {
      newLibelle = libelle;
    }
    if (nature == NatureService.unique) {
      try {
        if (prix != initialService.prix.toString()) {
          newPrix = double.parse(prix);
        }
      } catch (e) {
    ...
    if (description != (initialService.description)) {
    ...
    if (areTarifListsDifferent(
        list1: tarifModifies, list2: initialService.tarif)) {
      newTarif = tarifModifies;
    }

    if (_selectedCountry != initialService.country) {
      newCountry = _selectedCountry;
    }
    if (nature != initialService.nature) {
      newNature = nature;
    }
    if (type != initialService.type) {
      newType = type;
    }
    ...
    if (newLibelle == null &&
        ...
        newPrix == null &&
        (newTarif == null || newTarif.isEmpty) &&
        newCountry == null &&
        newType == null) {
      MutationRequestContextualBehavior.showCustomInformationPopUp(
